...
namespace caffe {
    ...
    template <typename Dtype>
        __global__ void DivBsx(const int nthreads, const Dtype* A,
                const Dtype* v, const int rows, const int cols, const
    ...
    template <typename Dtype>
        void NormalizeLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
            ...
            const Dtype* sum_channel_multiplier =
                sum_channel_multiplier_.gpu_data();
            int num = bottom[0]->num();
            int dim = bottom[0]->count() / num;
            int spatial_dim = bottom[0]->height() *
                bottom[0]->width();
            ...
            for (int n = 0; n < num; ++n) {
                ...
                if (across_spatial_) {
                    ...
                    caffe_gpu_gemv<Dtype>(CblasTrans,
                            ...
                            Dtype(1),
                            norm_data);
                    caffe_gpu_powx<Dtype>(spatial_dim,
                            norm_data,
                            Dtype(0.5),
                            norm_data);
                    // scale the layer NOLINT_NEXT_LINE(whitespace/operators)
                    DivBsx<Dtype>
                        <<<CAFFE_GET_BLOCKS(dim),
    ...
    template <typename Dtype>
        void NormalizeLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
            ...
            Dtype* buffer_channel =
                buffer_channel_.mutable_gpu_data();
            Dtype* buffer_spatial =
                buffer_spatial_.mutable_gpu_data();
            const Dtype* sum_channel_multiplier =
                sum_channel_multiplier_.gpu_data();
            const Dtype* sum_spatial_multiplier =
                sum_spatial_multiplier_.gpu_data();
            int count = top[0]->count();
            int num = top[0]->num();
            int dim = count / num;
            int spatial_dim = top[0]->height() *
                top[0]->width();
                    ...
                    if
                        ...
                        for
                             ...
                             ++n)
                            {
                                ...
                                caffe_gpu_div<Dtype>(channels,
                                        buffer_channel,
                                        scale,
                                        buffer_channel);
                                caffe_gpu_add<Dtype>(channels,
                                        buffer_channel,
                                        scale_diff,
                                        scale_diff);
                            }
                ...
                for (int n = 0; n < num;
                    ...
                    if
                        ...
                        caffe_gpu_gemv<Dtype>(CblasTrans,
                                ...
                                Dtype(0),
                                buffer_spatial);
                        // scale botom_diff NOLINT_NEXT_LINE(whitespace/operators)
                        MulBsx<Dtype>
                            <<<CAFFE_GET_BLOCKS(dim),
                            CAFFE_CUDA_NUM_THREADS>>>(
                                    dim,
                        ...
                        caffe_gpu_sub<Dtype>(dim,
                                ...
                                bottom_diff,
                                bottom_diff);
                        // divide by norm NOLINT_NEXT_LINE(whitespace/operators)
                        DivBsx<Dtype>
                            <<<CAFFE_GET_BLOCKS(dim),
                            CAFFE_CUDA_NUM_THREADS>>>(
                                    dim,
